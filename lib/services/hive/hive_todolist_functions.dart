...
class ToDoListHiveFunctions {
  ...
  static Map getSummary() => {
        ...
        'activeCount': todoListBox.values
            .where((item) => item['status'] == TaskStatus.active.index)
            .length,
        'doneCount': todoListBox.values
            .where((item) => item['status'] == TaskStatus.done.index)
            .length,
        'inProgressCount': todoListBox.values
            .where((item) => item['status'] == TaskStatus.inProgress.index)
            .length,
      };
  ...
  static Future<void> deleteTheseTasks(List keys) =>
      todoListBox.deleteAll(keys);
  static Future<void> deleteAllTasks() =>
      todoListBox.deleteAll(todoListBox.keys);
}
